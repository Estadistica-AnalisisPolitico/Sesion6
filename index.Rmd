...
-   **Verificando qué falta en el campo clave**: El merge usa columnas comunes. Antes del merge definitivo hay que verificar si hay correcciones posible para que el merge no pierda tantas filas. Esto es lo que IDH tiene pero que DEMO no tendría:

```{r}
setdiff(demo$Pais,idh$Pais)
```

Esto es lo que DEMO tiene pero que IDH no tendría:
...
Les daremos por nombre 'demos_efa' y 'desahu_efa' a esas dos columnas. Dado que tenemos el indice de democracia en la data original, comparémoslo con el recién calculado, via el *scatterplot* de la Figura \@ref(fig:scatterEFAdemo).
...
Veamos que tanto se parece el score obtenido via CFA con la punt
